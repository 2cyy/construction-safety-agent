...
请极其仔细地逐人检查，特别注意以下容易漏检的违规：

【吸烟检测 — 关键！】
- 逐人看手部：有没有夹持香烟？（香烟很小，但通常为白色细长条，夹在指间）
- 手拿香烟即使没有明显烟雾也算违规！
- 手靠近嘴部 + 指间有疑似烟头 → 标记为吸烟
- 不要只依赖"烟雾"判断——很多情况下手里有烟但没有可见烟雾
...
返回JSON格式（数值必须是整数）：
{"total_people": 总人数, "no_helmet": 未戴安全帽人数, "smoking": 0或1, "channel_blocked": 0或1, "material_issue": 0或1, "details": "描述所有违规的位置和类型"}"""


def qwen_scene_analysis(image_base64: str) -> dict:
    """千问引擎：场景级语义分析（只在高风险场景调用）"""
    messages = [
